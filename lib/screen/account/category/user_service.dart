...
class UserService extends StatefulWidget {
  @override
  State<StatefulWidget> createState() => _UserService();
}

class _UserService extends State<UserService> {
  ...
  Future<void> getList() async {
    try {
      ...
      final List<IdNameModel> itemList =
          await CategoryServiceController.getUserService();
  ...
  void onSearch(String value) async {
    ...
    if (value.length > 2) {
      ...
      final List<IdNameModel> itemList =
          await CategoryServiceController.getUserService(search: value);
      setState(() {
        activity = false;
        list = itemList;
      });
